...
switch($tanmatsu){
	default:	$html=load_log1('./pc.php','php');
				$res_tmp=load_log1('./data/res_pc.dat','html');
...
switch($post['status']){
	case 'preview':
		$form='<form action="./" method="POST" id="toiawase-form">'."\n";
		$form.='<h2 class="blog_post_title">入力確認ページ：資料請求フォーム</h2>';
		$form.=$table_s;
		$count=0;
	...
	$form.='<h2 class="blog_post_title">送信完了です</h2>'."\n";
	$count=0;
	foreach($names as $val){
		$option=explode(",",$val['option']);
		if($count==0){$tr_class=' class="top"';}else{$tr_class='';}
	...
	$form.='<p><strong>お問い合わせありがとうございました。</strong></p>
...
[<a href="./">お問合わせフォームへ戻る</a>]
	...
	foreach($html as $html_tmp){
		...
		$html_v.=$html_tmp;
		if(ereg('<form',$html_tmp) && ereg('id="toiawase-form"',$html_tmp)){
			$html_v.='<h2 class="blog_post_title">資料請求フォーム</h2>'.$error_div."\n";
		}
	}
	break;
}
