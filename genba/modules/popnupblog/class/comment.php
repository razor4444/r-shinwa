<?php
// $Id: comment.php,v 3.00 2006/12/15 11:01:56 yoshis Exp $
//  ------------------------------------------------------------------------ //
...
//			}
			...
			if($comment_uid > 0){
				$comments[$i]['name'] = XoopsUser::getUnameFromId($comment_uid,$xoopsModuleConfig['show_name']);
				if($xoopsUser){
	...
	function updateComment(&$blogid,&$postid,$comment_uid,&$comment_name, $comment_id, $comment='', $updatevote=0, $status, $notifypub=NULL){
		...
		return comment::GetStatusByCommentid($comment_id);
	}
