<?php
// $Id: PopnupBlogUtils.php,v 3.00 2006/12/15 11:01:56 yoshis Exp $
//  ------------------------------------------------------------------------ //
...
class PopnupBlogUtils {
	...
	function getDateFromHttpParams(){
		...
		$trackback = isset($_GET['trackback']) ? ($_GET['trackback']) : 0;
		if(!$postid) $postid = isset($_GET['postid']) ? intval($_GET['postid']) : 0;
		//if(!$trackback) $trackback = isset($_GET['trackback']) ? ($_GET['trackback']) : 0;
		if(!$param) $param = isset($_GET['param']) ? ($_GET['param']) : 0;
		if(!$param && !$postid){
	...
	function get_blog_list($start = 0, $cat_id=0){
		...
		if (!$start) $selectMax = "0," . POPNUPBLOG_BLOCK_LIST_NUM ;
		else $selectMax = intval($start). "," . POPNUPBLOG_BLOCK_LIST_NUM ;
		$wstr='';
		...
		$sql_select = sprintf('select uid, blogid,cat_id,group_read,last_update,title,blog_desc,ml_function FROM %s '.
			'WHERE last_update!=0 '.$wstr.' ORDER BY last_update desc limit %s',
		...
		while(list($result_uid,$result_bid,$result_cid,$group_read,$last_update,$title,$desc,$ml_function)=$xoopsDB->fetchRow($result_select)){
			...
			if (array_intersect($group_postarray,$ugroup_post) || $result_uid==$uid || in_array($result_bid,$pbids)){
					...
					if($rblog){
						...
						/*
						$rcomment = comment::get_RecentlyComment($result_bid);
						if($rcomment){
