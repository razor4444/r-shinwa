<?php
// $Id: popnupblog.php,v 3.03 2006/02/09 11:02:56 yoshis Exp $
//  ------------------------------------------------------------------------ //
...
//		return $this->targetUser->uname();
	...
	function setBlogInfo( $group_post="",$cat_id=0,$group_read="",$group_comment="",$group_vote="",$title="",$desc=""
		,$email='',$plugin='',$ml_function=0,$pop_server='',$pop_user='',$pop_password='',$pop_address='',$default_status=NULL){
		global $xoopsUser;
		$myts =& MyTextSanitizer::getInstance();
		if (!$title) $title = _MD_POPNUPBLOG_TITLE_PREFIX.$xoopsUser->uname()._MD_POPNUPBLOG_TITLE_SUFFIX;
		$title = $myts->addSlashes( $myts->censorString( $title ) );
		$title = htmlspecialchars($title);
		$sql = 'UPDATE '.PBTBL_INFO.' set cat_id='.intval($cat_id);
		$sql .= $group_post    ? ',group_post   =\''.$group_post   .'\'' : ',group_post   =NULL';
		$sql .= $group_read    ? ',group_read   =\''.$group_read   .'\'' : ',group_read   =NULL';
		$sql .= $group_comment ? ',group_comment=\''.$group_comment.'\'' : ',group_comment=NULL';
		$sql .= $group_vote    ? ',group_vote   =\''.$group_vote   .'\'' : ',group_vote   =NULL';
	...
	function getBlogData($postid=0, $year=0, $month=0, $date=0, $command=0, $limit=0, $voteorder=0,$status=1){
		...
		while(list($postid,$uid,$last_update,$post_date, $result_title,$result_post_text,$votes_yes,$votes_no)
			...
			$result['blog'][$i]['comments'] = comment::getComments($postid);
