<?php
// $Id: admin.php,v 1.1.1.1 2005/08/28 02:13:09 yoshis Exp $ 
// For Menu
define("_AM_POPNUPBLOG_PERMITIONERR", "non ouvert en écriture");
define("_AM_PREFERENCES","Préférences");
define("_AM_POPNUPBLOG_GOMOD","Aller au Module");
define("_AM_POPNUPBLOG_SUPPORTSITE","Site de Support");

// Main
define('_AM_POPNUPBLOG_CREATE', 'Cr&eacute;er un nouvel utilisateur du Blog');
define('_AM_POPNUPBLOG_EDIT', 'Editer');
define('_AM_POPNUPBLOG_OPERATION', 'OPERATION');
define('_AM_POPNUPBLOG_APPLICATED_USER', 'blog de r&eacute;f&eacute;rence');
define('_AM_POPNUPBLOG_BLOG_TITLE', 'Titre du BLOG');
define('_AM_POPNUPBLOG_REJECT_APPLICATION', 'rejet');
define('_AM_POPNUPBLOG_USER_LIST', 'Liste des Utilisateurs');
define('_AM_POPNUPBLOG_PERMISSION', 'Autorisations');
define('_AM_POPNUPBLOG_LASTUPDATE', 'Derni&egrave;re Mise &agrave; jour');
define('_AM_POPNUPBLOG_BLOG_EMAIL', 'Adresse Email de Post');	//add Yoshi.S
define('_AM_POPNUPBLOG_SEND_EMAIL', 'Envoyer &agrave; l\'adresse email');
define('_AM_POPNUPBLOG_ALLOW_APPLICATION', 'Autoris&eacute;e');
define('_AM_POPNUPBLOG_NEW_VERSION', 'Une nouvelle version est disponible!');
define('_AM_POPNUPBLOG_NAME', 'Nom');
define('_AM_VOTE', 'Vote');
define('_AM_COMMENT', 'Commentaire');
define('_AM_READ', 'Lecture');
define('_AM_POPNUPBLOG_UID', 'N°');
define('_AM_POPNUPBLOG_UNAME', 'Nom d\'utilisateur');
define('_AM_POPNUPBLOG_GROUP', 'Groupe:');
define('_AM_POPNUPBLOG_GROUPID', 'Groupe de Blog');
define('_AM_POPNUPBLOG_CANTADDEMAIL', 'Cette adresse email a d&eacute;j&agrave; &eacute;t&eacute; enregistr&eacute;e!');
//%%%%%%	Ported from newbb admin %%%%%
define('_AM_ADMIN','Administrateurs');
define('_AM_PRIVATE','Priv&eacute; (Admin)');
define('_AM_POPNUPBLOG_FORM_ADMIN_USAGE','Les utilisateurs priv&eacute;s sont enregistr&eacute;s dans la liste ci dessous.');
define('_AM_MODERATOR','Mod&eacute;rateur(s):');
define('_AM_BLOGCONF','Configuration des Blogs');
define('_AM_BLOGDESCRIPTION','Description du Blog');
define('_AM_ADDACAT','Ajouter une Cat&eacute;gorie');
define('_AM_LINK2ADDCAT','Ce lien vous permet d\'ajouter une cat&eacute;gorie afin de d&eacute;poser des blogs dedant.');
define('_AM_EDITCATTTL','Editer un titre de cat&eacute;gorie');
define('_AM_LINK2EDITCAT','Ce lien vous permet d\'&eacute;diter le titre de la cat&eacute;gorie en vue de le modifier.');
define('_AM_RMVACAT','Supprimer une cat&eacute;gorie');
define('_AM_LINK2RMVCAT','ce lien vous permet de supprimer n\'importe quelle cat&eacute;gorie de la base de donn&eacute;es');
define('_AM_REORDERCAT','Re-Ordonner les Cat&eacute;gories');
define('_AM_LINK2ORDERCAT','Ce lien vous permet de modifier l\'ordre dans lequel les cat&eacute;gories sont visualis&eacute;es dans la page index');
define('_AM_CONFIRM','Confirmer');
define('_AM_ADD','Ajout');
define('_AM_REMOVE','Supprimer');
define('_AM_NONE','Aucun');
define('_AM_CATEGORY','Cat&eacute;gorie :');
define('_AM_EDIT','Editer');
define('_AM_CATEGORYUPDATED','Cat&eacute;gorie mis &agrave; jour.');
define('_AM_EDITCATEGORY','Edition de cat&eacute;gorie :');
define('_AM_CATEGORYTITLE','Titre de Cat&eacute;gorie :');
define('_AM_SELECTACATEGORYEDIT','Selectionnez une cat&eacute;gorie &agrave; &eacute;diter');
define('_AM_CATEGORYCREATED','Cat&eacute;gorie cr&eacute;&eacute;e.');
define('_AM_NTWNRTFUTCYMDTVTEFS','Note : ceci ne supprimera pas les Blogs dans la ct&eacute;gorie, vous devez, pour ce faire par la section d\&eacute;dition.');
define('_AM_REMOVECATEGORY','Supprimer la Cat&eacute;gorie');
define('_AM_CREATENEWCATEGORY','Cr&eacute;er une nouvelle cat&eacute;gorie');
define('_AM_CATEGORYMOVEUP','Cat&eacute;gorie d&eacute;plac&eacute;e vers le haut');
define('_AM_TCIATHU','c\'est d&eacute;j&agrave; la cat&eacute;gorie la plus haute.');
define('_AM_CATEGORYMOVEDOWN','Cat&eacute;gorie d&eacute;plac&eacute;e vers le bas');
define('_AM_TCIATLD','c\'est d&eacute;j&agrave; le cat&eacute;gorie la plus basse.');
define('_AM_SETCATEGORYORDER','Param&egrave;tres de l\'ordre des cat&eacute;gories');
define('_AM_TODHITOTCWDOTIP','L\'ordre montr&eacute; ici est l\'ordre des cat&eacutegorie qui sera affich&eacute; sur la page index. pour déplacer l\'ordre d\'une cat&eacute;gorie cliquer sur monter pour monter d\'un niveau et sur descendre pour baisser d\'un niveau.');
define('_AM_ECWMTCPUODITO','chaque clic d&eacute;placera la cat&eacute;gorie d\'une place en haut ou en bas.');
define('_AM_CATEGORY1','Cat&eacute;gorie');
define('_AM_MOVEUP','Monter');
define('_AM_MOVEDOWN','Descendre');
define('_AM_CLEAR','Nettoyer');
define('_AM_SAVECHANGES','Sauvegarder les changements');
define('_AM_CATEGORYDELETED','Categorie effac&eacute;e.');
?>
