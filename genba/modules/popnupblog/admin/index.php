<?php
// $Id: index.php,v 3.00 2006/12/15 11:00:30 yoshis Exp $
//  ------------------------------------------------------------------------ //
...
if(isset($_POST['edit'])) {
	...
	if($targetUid > 0){
		$blog = new PopnupBlog($targetBid);
		$blog->setBlogInfo(
			$group_post,$cat_id,$group_read,$group_comment,$group_vote,$title,$desc,$email,$plugin,
...
<?php foreach($appList as $app){ ?>
	...
	<tr><td valign="top" class='even' align='center' width='20%'>
		...
		foreach($bloguids as $row){
			echo '<option value='.$row.'>'.$unames[$row].'</option>';
		}
		echo '</select>';
		?>
	...
	if(count($users) > 0){
		...
		<tr><form method="post" action="index.php">
			<tr><td class="head" align='left'>
				...
				<?php echo htmlspecialchars($user['user']->uname()); ?>
