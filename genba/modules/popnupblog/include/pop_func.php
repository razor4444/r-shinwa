<?php
// $Id: pop2blog.php,v 2.60 2006/11/10 09:16:08 yoshis Exp $
/**
Copyright(c) Yoshi Sakai @ Bluemoon inc. GPL licence
**/
...
//	$sqlDate =sprintf("%04d-%02d-%02d %02d:%02d:%02d",$dates['year'],$dates['mon'],$dates['mday'],$dates['hours'],$dates['minutes'],$dates['seconds']);
	...
	if ( preg_match("/^(re: \[)([^\:]+)([^0-9]+)([^\]]+)/i", $subject,$arg) ) {
		...
		$ret = comment::insertComment($blogid,$postid,$uid,$comment_name,$text,"",$status,1);
		if ($ret>0){
