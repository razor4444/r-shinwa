<?php
// $Id: notification.inc.php,v 1.1.1.1 2005/08/28 02:13:08 yoshis Exp $
//  ------------------------------------------------------------------------ //
...
function popnupblog_notify_iteminfo($category, $item_id)
{
	$module_handler =& xoops_gethandler('module');
	$module =& $module_handler->getByDirname('popnupblog');
	...
	if ($category=='blog') {
		...
		$sql = 'SELECT title FROM ' . $xoopsDB->prefix('popnupblog_info') . ' WHERE blogid = '.$item_id;
		$result = $xoopsDB->query($sql); // TODO: error check
		$result_array = $xoopsDB->fetchArray($result);
		$item['name'] = $result_array['title'];
		$item['url'] = XOOPS_URL . '/modules/' . $module->getVar('dirname') . '/index.php?param=' . $item_id;
		return $item;
	}
