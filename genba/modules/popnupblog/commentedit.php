<?php
// $Id: commentedit.php,v 3.07 2007/04/19 10:42:56 yoshis Exp $
require('header.php');
...
*/
	...
	$comment_name = isset($_POST['comment_name']) ? $_POST['comment_name'] : "";
		...
		if($postid) $blogid = PopnupBlogUtils::get_blogid_from_postid($postid);		if($comment_id) $blogid = comment::get_blogid_from_commentid($comment_id);	}
	...
	if($delete == 'on'){
		if(comment::updateComment($blogid,$postid,$comment_uid,$comment_name,$comment_id, null,0)){
		...
		if ($admin) $status = array_key_exists('status', $_POST) ? $_POST['status'] : 0;
			...
			$status = $ThisBlog->default_status;			$ret = comment::insertComment($blogid,$postid,$comment_uid,$comment_name,$comment_txt,$vote,$status,$notifypub);
		}else{			$ret = comment::updateComment($blogid,$postid,$comment_uid,$comment_name,$comment_id, $comment_txt, $vote,$status,$notifypub);		}
		if($ret){
			//
			// Sending Mails
			//
		...
		if($preview == 'on'){
			...
			$result = comment::getComment1($comment_id);
			if($result){
